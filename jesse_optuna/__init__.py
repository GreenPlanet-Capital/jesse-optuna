...
def create_config() -> None:
    ...
    shutil.copy2(
        f"{package_dir}/optuna_config.yml", f"{target_dirname}/optuna_config.yml"
    )
...
def run() -> None:
    ...
    cfg = get_config()
    study_name = (
        f"{cfg['strategy_name']}-{cfg['exchange']}-{cfg['symbol']}-{cfg['timeframe']}"
    )
...
def objective(trial):
    ...
    try:
        training_data_metrics = backtest_function(
    ...
    total_effect_rate = np.log10(training_data_metrics["total"]) / np.log10(
        cfg["optimal-total"]
    )
...
def get_candles_with_cache(
    ...
    if cache_file.is_file():
        ...
        start_dt, finish_dt = get_timestamp(start_date), get_timestamp(finish_date)
        candles = get_candles(
            exchange, symbol, "1m", start_dt, finish_dt, warmup_candles_num
        )
...
def backtest_function(start_date, finish_date, hp, cfg):
    ...
    candles_warmup_arr, candles_arr = get_candles_with_cache(
        cfg["exchange"],
        cfg["symbol"],
        start_date,
        finish_date,
    ...
    candles_warmup[jh.key(cfg["exchange"], cfg["symbol"])] = {
        "exchange": cfg["exchange"],
        "symbol": cfg["symbol"],
        "candles": candles_warmup_arr,
    }
